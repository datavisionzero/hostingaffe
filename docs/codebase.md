...
token, which is a key and not an identity), `Installations` (with the secret and
the file it lies in, and the dependency on another installation), `Files` (with the one list of refused paths),
...
`Status`, the lifecycle a machine and an installation share; `Port`, what an
installation and a machine alike listen on, with its two closed sets; `Anchor`,
the machine-or-installation a file is owned by and a page attached to; `Spelling`,
...
reading those tiles lead into — the whole record's history with the deployments
among it, walked by the cursor the endpoint hands out. `Reports.tsx` is the machine screen's other half — what the machine says about itself, the series of it, and
...
is written the moment it is made. Its grid of choices — one Tab stop, the
arrow keys walking the cells — is `Choices.tsx`, which the emblem chooser uses
as well.

`emblems/` is the same for a provider (ADR 0022): `drawings/` holds the
geometric compositions of the `emblem` set as SVG, each a tile of its own
whose three colours arrive as `--emblem-ground`, `--emblem-a` and
`--emblem-b`, and `ProviderEmblem.tsx` draws one. `emblems.ts` holds the ten
palettes and the emblem a key derives, with the avatar's hash and seeds of its
own; `EmblemChooser.tsx` is the chooser in the provider screen's header.
