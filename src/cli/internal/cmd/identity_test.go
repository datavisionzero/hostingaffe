...
func TestIdentityVerbsPrintSecretsOnceAndHitTheirEndpoints(t *testing.T) {
	...
	cases := []struct {
		...
		{[]string{"version"}, "GET", "/version", nil, "ha 0.0.0-dev\nhostingaffe 0.0.0-dev", ""},
		...
		{[]string{"agent", "create", "--name", "quiet-otter-42"}, "POST", "/agents", map[string]any{"name": "quiet-otter-42"}, "token: ha_secret-shown-once", "shown once"},
		...
		{[]string{"token", "create"}, "POST", "/tokens", nil, "token: ha_secret-shown-once", "shown once"},
		{[]string{"token", "list"}, "GET", "/tokens", nil, "ha_secre…", ""},
