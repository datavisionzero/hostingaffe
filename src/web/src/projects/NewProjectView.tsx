...
export function NewProjectView() {
  ...
  async function create(event: FormEvent<HTMLFormElement>) { event.preventDefault(); const data = new FormData(event.currentTarget); const result = await api.POST("/projects", { body: { key, name: String(data.get("name")) } }); // The frame first, then the URL: the shell is not remounted by navigation,
    // so a shell still on the old list has no current project on arrival and
    // draws every one of its links disabled.
    if (result.data) { await reload(); void navigate(`/${result.data.key}/pages`); } else setError(describe(result.error, result.response.status)); }
  return <><PageHeader title="Create project" /><form className="mx-auto grid w-full max-w-lg gap-4 p-6" onSubmit={(e) => void create(e)}><label className="text-sm">Key<Input name="key" value={key} onChange={(e) => setKey(e.target.value.toUpperCase())} required pattern="[A-Z][A-Z0-9]{1,9}" title="Two to ten characters: a letter, then letters and digits. Never changed afterwards." className="mt-1" /></label><label className="text-sm">Name<Input name="name" required className="mt-1" /></label>{error && <p role="alert" className="text-sm text-destructive">{error}</p>}<Button type="submit">Create project</Button></form></>;
}
